...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
    ...
    {"bot", G_admin_bot, "bot",
      "Add or remove bots",
      "[^3add/del^7] [^3name^7] (^5team^7) (^5skill^7)"
    },

    {"botcmd", G_admin_botcmd, "bot",
      "Command a bot",
      "[^3botname^7] [^3command^7]"
    },
    ...
    {"class", G_admin_class, "class",
      "change your player class",
      "[^3class^7]"
    },
    ...
    {"item", G_admin_item, "item",
      "give you any item you desire",
      "[^3item^7]"
    },
...
static void admin_default_levels( void )
{
  ...
  Q_strncpyz( g_admin_levels[ 0 ]->flags, 
    "listplayers admintest help specme time item speed gravity class", 
    sizeof( l->flags ) );
  ...
  Q_strncpyz( g_admin_levels[ 2 ]->flags, 
    "listplayers admintest help specme time putteam spec999 warn denybuild item speed gravity class",
    sizeof( l->flags ) );

  Q_strncpyz( g_admin_levels[ 3 ]->name, "^2Junior Admin",
    sizeof( l->name ) );
  Q_strncpyz( g_admin_levels[ 3 ]->flags, 
    "listplayers admintest help specme time putteam spec999 kick mute warn item speed gravity class"
  ...
  Q_strncpyz( g_admin_levels[ 4 ]->flags, 
    "listplayers admintest help specme time putteam spec999 kick mute showbans item speed gravity class "
...
void quicksell( gentity_t *ent, int item, char s[MAX_TOKEN_CHARS] )
{
  int       i;
  int       weapon, upgrade;

  if(item != WP_NONE) //We can assume that item is already "something"
    weapon = item;
  else
    upgrade = item;
  ...
  if( weapon != WP_NONE )
  {
    ...
    if( BG_InventoryContainsWeapon( weapon, ent->client->ps.stats ) )
    {
      ...
      if( ( weapon == WP_HBUILD || weapon == WP_HBUILD2 ) &&
          ent->client->ps.stats[ STAT_MISC ] > 0 )
      {
        trap_SendServerCommand( ent-g_entities, va( "print \"Cannot remove until build timer expires\n\"" ) );
        return;
      }
...
qboolean G_admin_item( gentity_t *ent, int skiparg ){
  ...
  else if( upgrade != UP_NONE )
  {
    ...
    if( upgrade == UP_BATTLESUIT && ent->client->ps.pm_flags & PMF_DUCKED )
    {
      trap_SendServerCommand( ent-g_entities,
        va( "print \"You can't obtain this item while crouching\n\"" ) );
      return qfalse;
    }
...
qboolean G_admin_class( gentity_t *ent, int skiparg ) 
{
  char      s[ MAX_TOKEN_CHARS ];
  int       clientNum;
  int       i;
  ...
  if( G_SayArgc() < 2 + skiparg )
  {
    ADMP( "^3!class: ^7usage: !class [class]\n" );
    return qfalse;
  }
  G_SayArgv( 1 + skiparg, s, sizeof( s ) );
...
qboolean G_admin_speed( gentity_t *ent, int skiparg )
{
  ...
  if( G_SayArgc() < 2 + skiparg )
  {
    ADMP( "^3!speed: ^7usage: !speed [speed]\n" );
    return qfalse;
  }
  ...
  if( speed >= 1 && speed <= 5) {
    ent->customspeed = speed;
    ent->customspeedset = qtrue;
    ADMP( va("^1Speed set to %d\n", speed) );
    return qtrue;
  }
  else
  {
    ADMP( "Speed value must be between 1 and 5 inclusive\n");
    return qfalse;
  }
  return qfalse;
}
...
qboolean G_admin_gravity( gentity_t *ent, int skiparg )
{
  ...
  if( G_SayArgc() < 2 + skiparg )
  {
    ADMP( "^3!gravity: ^7usage: !gravity [gravity]\n" );
    return qfalse;
  }
  G_SayArgv( 1 + skiparg, s, sizeof( s ) );
  ...
  if( grav >= -1000 && grav <= 1000) {
    ent->customgravity = grav;
    ent->customgravityset = qtrue;
    ADMP( va("^1Gravity set to %d\n", grav) );
    return qtrue;
  }
  else
  {
    ADMP( "Gravity value must be between 0 and 1000 inclusive\n");
    return qfalse;
  }
  return qfalse;
}
